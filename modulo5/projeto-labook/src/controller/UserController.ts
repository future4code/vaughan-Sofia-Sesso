...
import { ManagingFriendshipInputDTO, InterfaceUserController, LoginInputDTO, SignupInputDTO } from '../model/User'
...
export class UserController implements InterfaceUserController {
    ...
    public unfriend = async (req: Request, res: Response): Promise<void | Response> => {
        try {
            ...
            const friendName: string = await this.userBusiness.unfriend(input)

            res.status(200).send(`Usuário removeu ${friendName} da sua lista de amigos!`)
        }
        catch (error) {
            if (error instanceof CustomError) {
                return res.status(error.statusCode).send(error.message)
            }
            res.status(500).send("Erro ao remover amigo")
        }
    }
}